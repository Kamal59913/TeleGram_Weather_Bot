...
export const SimpleSearch = <T,>({
  ...
  const showResetButton = inputValue || searchValue || foundData;
      ...
      <div className="relative">
        <div className="relative group">
          ...
          <input
            ...
            className={`w-full h-11 pl-12 ${showResetButton ? "pr-32 sm:pr-36" : "pr-24 sm:pr-28"} rounded-xl bg-white text-black placeholder:text-gray-400 border-2 transition-all duration-300 shadow-sm hover:shadow-md focus:shadow-lg dark:bg-black dark:text-white dark:placeholder:text-gray-500 ${
              error
          ...
          <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1.5 sm:gap-2">
            ...
            {showResetButton && (
              <button
                onClick={handleReset}
                className="p-2 sm:p-2.5 bg-gray-100 hover:bg-gray-200 dark:bg-black dark:hover:bg-gray-700 text-gray-600 hover:text-black dark:text-gray-400 dark:hover:text-gray-200 rounded-lg transition-all duration-200 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-gray-400/50 group flex-shrink-0"
                title="Clear all"
              >
                <svg
            ...
            <button
              ...
              className="bg-[#4a90d9] hover:bg-gray-600 dark:bg-white dark:from-white dark:to-gray-100 dark:hover:from-gray-100 dark:hover:to-white disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none duration-200 flex focus:outline-none focus:ring-2 focus:ring-[#9B3B97]/50 font-semibold gap-1.5 sm:gap-2 items-center px-2.5 sm:px-3.5 py-2 rounded-lg text-xs sm:text-sm text-white dark:text-black flex-shrink-0"
            >
              {isLoading ? (
                <>
                  <div className="w-3.5 h-3.5 sm:w-4 sm:h-4 border-2 border-white/30 dark:border-gray-800/30 border-t-white dark:border-t-black rounded-full animate-spin" />
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-green-700 dark:text-green-300 mt-1">
                  Data retrieved for:{" "}
                  <span className="font-mono font-semibold">{inputValue}</span>
                </p>
              </div>
            </div>
          </div>
