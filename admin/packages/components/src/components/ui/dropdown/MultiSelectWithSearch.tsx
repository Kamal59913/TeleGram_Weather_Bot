...
interface MultiSelectDropdownProps {
  ...
  setFetchedCountries:React.Dispatch<React.SetStateAction<[]>>;
  isOpen: boolean;
...
const MultiSelectDropdown: React.FC<MultiSelectDropdownProps> = ({
  ...
  const removeOption = (
    ...
    setFetchedCountries((prev: any) =>
      prev.filter((item: any) => item.country.toLowerCase() !== option.label.toLowerCase())
    );
  ...
  return (
    <div className="flex items-center gap-2">
      <div className={`${width} relative`} ref={dropdownRef}>
        <div
          className={`bg-white dark:bg-white/[0.03] border border-gray-300 dark:border-gray-800 rounded-lg px-4 flex flex-wrap items-center min-h-12 cursor-pointer shadow-sm focus:outline-none focus:ring-2 transition duration-150 overflow-auto ${
            isOpen
        ...
        >
          ...
          {selectedOptions.slice(0, 200).map((option: Option) => (
            <div
              key={option.value}
        ...
        {isOpen && (
          <div className="absolute mt-1 w-full bg-white dark:bg-black border border-gray-200 dark:border-gray-800 rounded-lg shadow-lg z-10 max-h-100 overflow-y-auto">
      ...
      {/* Separate search button outside the main component */}
      <button
        onClick={openSearch}
        className="h-12 min-h-12 flex items-center justify-center px-3 rounded-lg bg-white dark:bg-white/[0.03] border border-gray-300 dark:border-gray-800 shadow-sm hover:border-gray-400 dark:hover:border-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
